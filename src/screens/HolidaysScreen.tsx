...
export default function HolidaysScreen({ navigation }: any) {
    ...
    const getKandilCelebrationMessage = () => {
        ...
        return language === 'tr'
            ? `Bugün ${name}! Hayırlı kandiller.`
            : language === 'ar'
            ? `اليوم ${name}! كل عام وأنتم بخير`
            : language === 'de'
            ? `Heute ist ${name}! Gesegnete Nacht.`
            : language === 'fr'
            ? `Aujourd'hui c'est ${name}! Bonne fête.`
            : language === 'id'
            ? `Hari ini ${name}! Selamat merayakan.`
            : language === 'ur'
            ? `آج ${name} ہے! مبارک ہو`
            : `Today is ${name}! Blessed night.`;
    };
